...
int main(int argc, char** argv, char** envp) {
    ...
    if (argc > 1 && !memcmp(argv[1], "Child", 6)) {
        ...
        size = sizeof(buffer1);
        DkStreamWrite(pal_public_state.parent_process, 0, &size, buffer1, NULL);

        size = sizeof(buffer1);
        ret = DkStreamWrite(pal_public_state.parent_process, 0, &size, buffer1, NULL);
        if (ret == 0 && size > 0)
            pal_printf("Process Write 1 OK\n");

        size = sizeof(buffer4);
        ret = DkStreamRead(pal_public_state.parent_process, 0, &size, buffer4, NULL, 0);
        if (ret == 0 && size > 0)
