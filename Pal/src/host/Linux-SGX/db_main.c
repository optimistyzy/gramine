...
noreturn void pal_linux_main(char* uptr_libpal_uri, size_t libpal_uri_len, char* uptr_args,
    ...
    /* seal-key initialization must come before protected-files initialization */
    if ((ret = init_seal_key()) < 0) {
        log_error("Failed to initialize SGX sealing key parameters: %d", ret);
        ocall_exit(1, /*is_exitgroup=*/true);
    }
