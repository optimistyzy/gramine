...
bool stataccess(struct stat* stat, int acc) {
    ...
    if (g_pal_common_state.host_euid && g_pal_common_state.host_euid == stat->st_uid) {
        mode >>= 6;
        goto out;
    }

    if (g_pal_common_state.host_egid && g_pal_common_state.host_egid == stat->st_gid) {
        mode >>= 3;
        goto out;
    }

    if (!g_pal_common_state.host_euid)
        mode >>= 6;
