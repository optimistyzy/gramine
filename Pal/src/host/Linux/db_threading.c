 ...
 * pal_public_state.user_address. Linux PAL also cannot just use malloc/free because DkThreadExit
...
int _DkThreadResume(PAL_HANDLE thread_handle) {
    int ret = DO_SYSCALL(tgkill, g_pal_common_state.host_pid, thread_handle->thread.tid, SIGCONT);
